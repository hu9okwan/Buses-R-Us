...
public class ArrivalsListFragment extends ListFragment {
    ...
    private ArrayList<Arrival> getArrivalsForSelectedStop() {
        ...
        while (i.hasNext()) {
            Arrival arrival = (Arrival) i.next();
            arrivalList.add(arrival);


            int nextIndex = 1;

            if (arrivalList.size() > 1) {

                for (Arrival a : arrivalList) {

                    if (arrivalList.size() == nextIndex)
                        break;

                    Arrival nextArrival = arrivalList.get(nextIndex);

                    if (a.compareTo(nextArrival) > 0) {

                        arrivalList.set(arrivalList.indexOf(a), nextArrival);
                        arrivalList.set(nextIndex, a);
