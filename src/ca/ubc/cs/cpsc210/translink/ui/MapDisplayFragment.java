        ...
        if (s != null){
            ...
            for (Route r : s.getRoutes()){
                ...
                for (RoutePattern rp : r.getPatterns()){
                    ...
                    LatLon previous = rp.getPath().get(0);
                    for (LatLon latLon : rp.getPath()){
                        ...
                        GeoPoint gp = new GeoPoint(previous.getLatitude(), previous.getLongitude());
                        GeoPoint gp2 = new GeoPoint(latLon.getLatitude(), latLon.getLongitude());

                        if (Geometry.rectangleIntersectsLine(northWest, southEast, previous, latLon)){
                            if (!listOfGeoPoints.contains(gp))
                                listOfGeoPoints.add(gp);
                            if (!listOfGeoPoints.contains(gp2))
                                listOfGeoPoints.add(gp2);

                        }

                        polyline.setPoints(listOfGeoPoints);
                        busRouteOverlays.add(polyline);
                        listOfGeoPoints.clear();
...
//                + southeast.getLatitude() + " " + southeast.getLongitude());
        ...
        for (Stop s : StopManager.getInstance()){
            ...
            if (Geometry.rectangleContainsPoint(northWest, southEast, s.getLocn())){
                ...
                if (stopMarkerMap.get(s) == null) {
                    ...
                    m.setTitle(s.getNumber() + " - " + s.getName() + routes );
    ...
    private void updateMarkerOfNearest(Stop nearest) {
        ...
        for (Marker m : stopMarkerMap.values()){
            if (m == nearestStopMarker)
                m.setIcon(stopIconDrawable);
        }


        if (nearestStopMarker == null || getMarker(nearest) == null) {
    ...
    private void handleLocationChange(Location location) {
        ...
        Stop nearest = StopManager.getInstance().findNearestTo(locationLatLon);

        updateMarkerOfNearest(nearest);

        locationListener.onLocationChanged(nearest, locationLatLon);
    }
